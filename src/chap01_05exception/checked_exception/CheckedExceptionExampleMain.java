...
public class CheckedExceptionExampleMain { // CheckedException : 컴파일시 예외처리를 문법적으로 강제한다.
    public static void main(String[] args) {
        Client client = new Client();

        // 해당 메서드가 throws로 본인을 호출하는 지역에서 예외처리를 하라고 떠넘긴 상태이므로
        // 현재 호출하고있는 CheckedExceptionExampleMain에서 try~catch를 작성해야함
        try {
        ...
        // 호출구역에서 따로 try~catch를 처리해줄 필요가 없음
